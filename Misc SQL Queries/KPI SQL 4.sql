...
select case 
	...
	, count(n.id) CT
	, cast(getdate() as date) Date_Captured-- into ICSC_Monthly_KIA_Capture_Rejoined_Members
from name n
...
where n.status = 'A'
	...
	and month(d.REJOIN_DATE) = month(getdate())
	and year(d.REJOIN_DATE) = year(getdate())
group by case 
	when n.MEMBER_TYPE like 'PB%' then 'Public Entities (Official and Affiliate)'
	when n.MEMBER_TYPE like 'AC%' then 'Academic (Official and Affiliate)'
	when n.MEMBER_TYPE like 'SM' then 'Students'
